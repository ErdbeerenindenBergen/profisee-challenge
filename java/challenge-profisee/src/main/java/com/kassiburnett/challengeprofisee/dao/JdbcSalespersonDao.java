...
public class JdbcSalespersonDao implements SalespersonDao {
    ...
    public Employee updateSalesperson(Employee employee) {
        String sql = "UPDATE employee SET id_of_manager = ?, first_name = ?, last_name = ?, " +
                " street_address = ?, town = ?, employee_state = ?, zip_code = ?, phone = ?, " +
                " email_address = ?, start_date = ?, termination_date = ? " +
                " WHERE employee_id = ?;";
        jdbcTemplate.update(sql, employee.getIdOfManager(), employee.getFirstName(), employee.getLastName(),
                employee.getStreetAddress(), employee.getTown(), employee.getState(),
                employee.getZipCode(), employee.getPhone(), employee.getEmailAddress(),
                employee.getStartDate(), employee.getTerminationDate(), employee.getEmployeeId());
        return employee;
    }
